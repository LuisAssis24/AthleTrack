...
class EventoController(
    ...
    @PostMapping("/apagar")
    @Transactional
    fun apagarEvento(@RequestBody request: EventoApagarRequestDTO): ResponseEntity<String> {
        return try {
            val professor = userRepository.findById(request.idProfessor)
                .orElseThrow { IllegalArgumentException("Professor não encontrado.") }
